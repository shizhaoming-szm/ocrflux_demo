...
class OcrFlux:
  ...
  async def process_task(args, worker_id, task_name, task_args):
      ...
      attempt = 0
      await OcrFlux.tracker.track_work(worker_id, f"{worker_id}", "started")
      ...
      while attempt < MAX_RETRIES:
          ...
          if task_name == 'page_to_markdown':
              ...
              query = OcrFlux.build_page_to_markdown_query(args, pdf_path, page_number, args.target_longest_image_dim, image_rotation=local_image_rotation)
          elif task_name == 'element_merge_detect':
              # Stage 2: 元素合并检测
              text_list_1,text_list_2 = task_args
              query = OcrFlux.build_element_merge_detect_query(args, text_list_1, text_list_2)
          elif task_name == 'html_table_merge':
              # Stage 3: HTML表格合并
              table_1,table_2 = task_args
              query = OcrFlux.build_html_table_merge_query(args, table_1, table_2)
          ...
          try:
              # 发送请求到vLLM服务器
              status_code, response_body = await OcrFlux.apost(COMPLETION_URL, json_data=query)
              ...
              # 标记任务完成
              await OcrFlux.tracker.track_work(worker_id, f"{worker_id}", "finished")
              return return_data
          
          except (ConnectionError, OSError, asyncio.TimeoutError) as e:
              # 网络连接错误，使用指数退避而不计入页面重试次数
              OcrFlux.logger.warning(f"Client error on attempt {attempt} for {worker_id}: {type(e)} {e}")

              # 指数退避策略：页面重试用于修复模型的错误结果，但vLLM请求应该正常工作
              # 如果出现连接错误，可能是服务器正在重启
              sleep_delay = 10 * (2**exponential_backoffs)
              exponential_backoffs += 1
              OcrFlux.logger.info(f"Sleeping for {sleep_delay} seconds on {worker_id} to allow server restart")
              await asyncio.sleep(sleep_delay)
          except asyncio.CancelledError:
              # 任务被取消
              OcrFlux.logger.info(f"Process {worker_id} cancelled")
              await OcrFlux.tracker.track_work(worker_id, f"{worker_id}", "cancelled")
              raise
          except json.JSONDecodeError as e:
              # JSON解析错误，计入重试次数
              OcrFlux.logger.warning(f"JSON decode error on attempt {attempt} for {worker_id}: {e}")
              attempt += 1
          except ValueError as e:
              # 值错误，计入重试次数
              OcrFlux.logger.warning(f"ValueError on attempt {attempt} for {worker_id}: {type(e)} - {e}")
              attempt += 1
          except Exception as e:
              # 其他未预期的错误，计入重试次数
              OcrFlux.logger.exception(f"Unexpected error on attempt {attempt} for {worker_id}: {type(e)} - {e}")
              attempt += 1

      # 所有重试都失败
      OcrFlux.logger.error(f"Failed to process {worker_id} after {MAX_RETRIES} attempts.")
      await OcrFlux.tracker.track_work(worker_id, f"{worker_id}", "errored")
  ...
  async def process_pdf(args, worker_id: int, pdf_path: str):
      ...
      6. 构建最终文档文本并返回结果
      """
      OcrFlux.logger.info(f"Start process_pdf for {pdf_path}")
      ...
      if pdf_path.lower().endswith(".pdf"):
          try:
              ...
              OcrFlux.logger.exception(f"Could not count number of pages for {pdf_path}, aborting document")
              return None
      else:
          # 图像文件（如PNG、JPG等）默认为单页处理
          num_pages = 1
      
      OcrFlux.logger.info(f"Got {num_pages} pages to do for {pdf_path} in worker {worker_id}")

      try:
          # ========== Stage 1: Page to Markdown 处理阶段 ==========
          # 批量处理所有页面，将每页转换为Markdown格式
          # results = []
          # batch_size = getattr(args, 'batch_size', 1)  # 获取批处理大小，默认为1
          
          # 按批次处理页面
          results = []
          batch_size = getattr(args, 'batch_size', 4)  # 从参数获取批处理大小，默认为4
          for i in range(0, num_pages, batch_size):
              tasks = []
              async with asyncio.TaskGroup() as tg:
                  # 为当前批次的每个页面创建异步任务
                  for page_num in range(i + 1, min(i + batch_size, num_pages) + 1):
                      task = tg.create_task(OcrFlux.process_task(args, worker_id, task_name='page_to_markdown', task_args=(pdf_path, page_num)))
                      tasks.append(task)
          ...
          for i,result in enumerate(results):
              if result != None:
                  page_number = i+1
                  # 后处理Markdown文本并按段落分割
                  page_to_markdown_result[i+1] = OcrFlux.postprocess_markdown_text(args,result,pdf_path,page_number).split("\n\n")
          ...
          async with asyncio.TaskGroup() as tg:
              ...
              for page_1,page_2 in page_pairs:
                  ...
                  task = tg.create_task(OcrFlux.process_task(args, worker_id, task_name='element_merge_detect', task_args=(page_to_markdown_result[page_1], page_to_markdown_result[page_2])))
                  tasks.append(task)
          ...
          while i < len(table_pairs):
              async with asyncio.TaskGroup() as tg:
                  ...
                  task = tg.create_task(OcrFlux.process_task(args, worker_id, task_name='html_table_merge', task_args=(tmp_page_to_markdown_result[page_1][elem_idx_1], tmp_page_to_markdown_result[page_2][elem_idx_2])))
                  tasks.append(task)
          ...
          document_text = OcrFlux.build_document_text(page_to_markdown_result, element_merge_detect_result, html_table_merge_result)
          ...
          if isinstance(e, ExceptionGroup):
              ...
              if broken_pool is not None:  # Found at least one BrokenProcessPool
                  OcrFlux.logger.critical("Encountered BrokenProcessPool, exiting process.")
                  sys.exit(1)

          OcrFlux.logger.exception(f"Exception in process_pdf for {pdf_path}: {e}")
          return None
  ...
  async def process_json(args, worker_id: int, json_path: str):
      ...
      try:
          ...
          OcrFlux.logger.exception(f"Could not load {json_path}, aborting document")
      ...
      try:
          ...
          if args.task == 'merge_pages':
              ...
              async with asyncio.TaskGroup() as tg:
                  task = tg.create_task(OcrFlux.process_task(args, worker_id, task_name='element_merge_detect', task_args=(page_1, page_2)))
              result = task.result()
              return {
                  "orig_path": json_path,
              ...
              async with asyncio.TaskGroup() as tg:
                  task = tg.create_task(OcrFlux.process_task(args, worker_id, task_name='html_table_merge', task_args=(table_1, table_2)))
              result = task.result()
              return {
                  "orig_path": json_path,
          ...
          OcrFlux.logger.exception(f"Exception in process_json for {json_path}: {e}")
          return None
  ...
  async def worker(args, work_queue: WorkQueue, semaphore, worker_id):
      ...
      while True:
          ...
          if work_item is None:
              OcrFlux.logger.info(f"Worker {worker_id} exiting due to empty queue")
              semaphore.release()
              break

          OcrFlux.logger.info(f"Worker {worker_id} processing work item {work_item.hash}")
          # 清理工作进程的状态追踪
          await OcrFlux.tracker.clear_work(worker_id)
          ...
          try:
              ...
              async with asyncio.TaskGroup() as tg:
                  if args.task == 'pdf2markdown':
                      ...
                      tasks = [tg.create_task(OcrFlux.process_pdf(args, worker_id, pdf_path)) for pdf_path in work_item.work_paths]
                  elif args.task == 'merge_pages' or args.task == 'merge_tables':
                      # 独立合并任务：处理JSON文件，执行特定阶段的合并操作
                      tasks = [tg.create_task(OcrFlux.process_json(args, worker_id, json_path)) for json_path in work_item.work_paths]
                  else:
                      raise ValueError(f"Unknown task {args.task}")

                  OcrFlux.logger.info(f"Created all tasks for {work_item.hash}")

              OcrFlux.logger.info(f"Finished TaskGroup for worker on {work_item.hash}")
              ...
              OcrFlux.logger.info(f"Got {len(results)} docs for {work_item.hash}")
              ...
              OcrFlux.logger.exception(f"Exception occurred while processing work_hash {work_item.hash}: {e}")
          finally:
              # 第七步：无论成功或失败，都要释放信号量
              # 确保其他等待的工作进程能够继续执行
              semaphore.release()
  ...
  # vllm_server_task函数已删除 - 不再启动vLLM服务器

  # vllm_server_host函数已删除 - 不再管理vLLM服务器生命周期
  ...
  async def vllm_server_ready(args):
      ...
      - 成功条件：HTTP状态码200
      """
      max_attempts = 100  # 进一步减少尝试次数
      delay_sec = 3  # 增加间隔时间
      url = f"http://{args.server_host}:{args.server_port}/v1/models"
      print(f"Checking vllm server status at {url}...")
      
      consecutive_failures = 0
      for attempt in range(1, max_attempts + 1):
          try:
              async with httpx.AsyncClient(timeout=15.0) as session:  # 增加超时时间
                  response = await session.get(url)
                  print('*'*20)
                  print(f"Status code: {response.status_code}")
                  if response.status_code == 200:
                      OcrFlux.logger.info("vllm server is ready.")
                      return
                  else:
                      print(f"Response text: {response.text[:200]}...")  # 显示响应内容的前200字符
                      OcrFlux.logger.warning(f"Attempt {attempt}: Unexpected status code {response.status_code}")
                      consecutive_failures = 0  # 重置连续失败计数
          except Exception as e:
              consecutive_failures += 1
              error_msg = str(e)[:100]
              OcrFlux.logger.warning(f"Attempt {attempt}: Connection failed - {error_msg}...")
              
              # 如果连续5次连接失败，提前退出
              if consecutive_failures >= 5:
                  print(f"\n连续{consecutive_failures}次连接失败，请检查：")
                  print(f"1. vllm服务器是否在 {args.server_host}:{args.server_port} 上运行")
                  print(f"2. 网络连接是否正常")
                  print(f"3. 防火墙设置是否阻止了连接")
                  raise Exception(f"无法连接到vllm服务器 {url}，连续{consecutive_failures}次失败")

          if attempt < max_attempts:  # 避免最后一次尝试后还要等待
              print(f"等待{delay_sec}秒后重试...")
  ...
  async def download_model(model_name_or_path: str):
      ...
      - HuggingFace模型ID：如 OCRFlux-3B
      """
      if os.path.isabs(model_name_or_path) and os.path.isdir(model_name_or_path):
          # 使用本地模型路径
          OcrFlux.logger.info(f"Using local model path at '{model_name_or_path}'")
      else:
          # 从HuggingFace下载模型
          OcrFlux.logger.info(f"Downloading model with hugging face '{model_name_or_path}'")
  ...
  async def metrics_reporter(work_queue):
      ...
      while True:
          # 输出队列状态和性能指标
          # 前导换行符保持表格格式在日志中的可读性
          OcrFlux.logger.info(f"Queue remaining: {work_queue.size}")
          OcrFlux.logger.info("\n" + str(OcrFlux.metrics))
          OcrFlux.logger.info("\n" + str(await OcrFlux.tracker.get_status_table()))
  ...
  async def main(args):
      ...
      Args:
          args: 配置参数对象，包含所有必需的配置项
      """
      if args is None:
          raise ValueError("args parameter is required. Use run_with_config() for dictionary-based configuration.")
      ...
      if args.task == 'pdf2markdown':
          ...
          for pdf_path in args.data:
              if os.path.exists(pdf_path):
                  ...
                  if pdf_path.lower().endswith(".pdf") and open(pdf_path, "rb").read(4) == b"%PDF":
                      OcrFlux.logger.info(f"Loading file at {pdf_path} as PDF document")
                      pdf_work_paths.add(pdf_path)
                  # 检查图像文件：支持常见图像格式
                  elif is_image(pdf_path):
                      OcrFlux.logger.info(f"Loading file at {pdf_path} as image document")
                      pdf_work_paths.add(pdf_path)
                  else:
          ...
          OcrFlux.logger.info(f"Found {len(pdf_work_paths):,} total pdf paths to add")
          ...
          for pdf_path in tqdm(sampled_pdfs, desc="Sampling PDFs to calculate optimal length"):
              try:
                  ...
                  OcrFlux.logger.warning(f"Failed to read {pdf_path}: {e}")

          # 计算平均页数，用于优化任务分组
          if page_counts:
              avg_pages_per_pdf = sum(page_counts) / len(page_counts)
          else:
              OcrFlux.logger.warning("Could not read any PDFs to estimate average page count.")
              avg_pages_per_pdf = 10  # 采样失败时的默认值

          # 根据目标页数和平均页数计算每组文件数
          items_per_group = max(1, int(args.pages_per_group / avg_pages_per_pdf))
          OcrFlux.logger.info(f"Calculated items_per_group: {items_per_group} based on average pages per PDF: {avg_pages_per_pdf:.2f}")
          ...
          for json_path in args.data:
              if os.path.exists(json_path):
                  ...
                  # 包含路径列表的文本文件
                      ...
                      OcrFlux.logger.info(f"Loading file at {json_path} as list of paths")
      ...
      OcrFlux.logger.info(f"Starting pipeline with PID {os.getpid()}")
      ...
      if qsize == 0:
          OcrFlux.logger.info("No work to do, exiting")
          return
      ...
      # vLLM服务器启动代码已删除 - 假设外部vLLM服务器已运行
      # 等待服务器就绪（保留检查功能）
      await OcrFlux.vllm_server_ready(args)

      # 启动指标监控任务
      metrics_task = asyncio.create_task(OcrFlux.metrics_reporter(work_queue))

      # 创建工作进程池：并发处理队列中的任务
      worker_tasks = []
      for i in range(args.workers):
          task = asyncio.create_task(OcrFlux.worker(args, work_queue, semaphore, worker_id=i))
      ...
      # 清理：取消监控任务
      metrics_task.cancel()
      OcrFlux.logger.info("Work done")
      
      # 工作完成后清理工作空间
      try:
          if os.path.exists(args.workspace):
              OcrFlux.logger.info(f"Cleaning up workspace: {args.workspace}")
              shutil.rmtree(args.workspace)
              OcrFlux.logger.info("Workspace cleanup completed")
      except Exception as e:
          OcrFlux.logger.warning(f"Failed to cleanup workspace {args.workspace}: {e}")
...
# 注意：此模块不支持直接命令行调用
# 请使用 run_with_config() 函数或直接调用 main() 函数

# 编程方式调用示例函数
async def run_with_config(config_dict):
    """
    以编程方式运行 OCRFlux，接受配置字典
    
    Args:
        config_dict: 包含所有配置参数的字典
    ...
    # 创建一个 Namespace 对象来模拟 argparse 的结果
    ...
    defaults = {
        ...
        'server_port': 8004,
        'batch_size': 5  # 批处理大小，控制每次处理的页数
    }
    
    # 合并默认值和用户配置
    for key, default_value in defaults.items():
        setattr(args, key, config_dict.get(key, default_value))
    
    # 生成随机唯一的工作空间路径
    workspace_name = f"ocrflux_workspace_{uuid.uuid4().hex[:8]}"
    args.workspace = os.path.join(os.getcwd(), workspace_name)
    
    # 确保工作空间目录存在
    os.makedirs(args.workspace, exist_ok=True)
    print(f"Generated workspace: {args.workspace}")
    
    # 调用 main 函数并获取返回结果
    result = await OcrFlux.main(args)
    print(f"OCRFlux处理结果: {result}")
    return result
